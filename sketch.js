...
function endSurge() {
  ...
  shuffle(available, true);
  let count = floor(
    random(UNCERTAIN_ZONE_COUNT_MIN, UNCERTAIN_ZONE_COUNT_MAX + 1),
  );
  count = min(count, available.length);
  for (let i = 0; i < count; i++) {
    available[i].isUncertainZone = true;
    available[i].uncertainTimer = random(
      UNCERTAIN_DURATION_MIN,
      UNCERTAIN_DURATION_MAX,
    );
  }
  let interval = lerp(
    SURGE_MAX_INTERVAL,
...
function updateSurge(dt) {
  if (surgeActive) {
    ...
    if (
      surgeNextIn <= 0 &&
      surgeCooldownLeft <= 0 &&
      timer < ROUND_DURATION - GRACE_PERIOD
    )
      startSurge();
  }
...
function updateGrounding(dt) {
  groundingTimer += dt;
  let beatPhase =
    (groundingTimer % GROUNDING_BEAT_INTERVAL) / GROUNDING_BEAT_INTERVAL;
...
function groundingInput() {
  ...
  if (diff <= GROUNDING_BEAT_WINDOW) {
    ...
    groundingHits++;
    groundingFeedback =
      diff <= GROUNDING_BEAT_WINDOW * 0.4 ? "Perfect!" : "Steady!";
...
function drawBed(bed) {
  ...
  if (bed.isWilted) fill(COL.bedDead);
    ...
    fill(
      COL.bedUncertain[0] + s,
      COL.bedUncertain[1] + s,
      COL.bedUncertain[2] + s,
    );
  ...
  if (bed.isUncertainZone) {
    noFill();
    stroke(
      COL.bedUncertain[0] + 40,
      COL.bedUncertain[1] + 40,
      COL.bedUncertain[2] + 40,
...
function drawPanel() {
  ...
  if (sb && !sb.isWilted) {
    textSize(12);
    fill(COL.textPrimary);
    text("Health: " + floor(sb.health), px, py);
    py += 16;
    fill(COL.water);
    text("Water:  " + floor(sb.water), px, py);
    py += 16;
    fill(COL.light);
    text("Light:  " + floor(sb.light), px, py);
    py += 16;
    if (sb.airflowActive) {
      fill(COL.airflow);
...
function drawActionButtons(px, py, pw) {
  ...
  for (let btn of actionBtns) {
    ...
    if (btn.id === "ground")
      available = groundingCooldownLeft <= 0 && gameState === STATE.PLAYING;
...
function drawGrounding() {
  ...
  if (groundingBeatIndex < GROUNDING_BEATS && !groundingComplete) {
    ...
    beatPhase =
      1.0 - constrain(abs(dist) / (GROUNDING_BEAT_INTERVAL * 0.5), 0, 1);
  } else beatPhase = 0.5;
  ...
  for (let i = 0; i < GROUNDING_BEATS; i++) {
    ...
    else if (groundingBeats[i]?.result && groundingBeats[i].result !== "")
  ...
  textSize(11);
  text(
    groundingHits + "/" + GROUNDING_SUCCESS_THRESHOLD + " needed",
    cx,
    cy + 150,
  );
...
function drawWin() {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    ellipse(
      cx + cos(t * 0.2 + i * 0.5) * 300,
      CANVAS_H / 2 + sin(t * 0.3 + i * 0.7) * 200,
      60,
      60,
    );
  }
...
function drawEndStats(cx, y) {
  ...
  textAlign(CENTER, CENTER);
  fill(COL.textPrimary);
  text("Score: " + floor(score), cx, y);
  y += lh;
  fill(COL.combo);
  text("Best Harmony: x" + nf(stats.peakCombo, 1, 1), cx, y);
  y += lh;
  fill(COL.surge);
  text("Surges Survived: " + stats.surgesSurvived, cx, y);
  y += lh;
  fill(COL.grounding);
  text(
    "Grounding: " +
  ...
  y += lh;
  fill(COL.accent);
  text("Plants Restored: " + stats.totalRestores, cx, y);
  y += lh;
  fill(COL.healthLow);
